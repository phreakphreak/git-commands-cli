...
command
  ...
  .action(async (option) => {
    ...
    rl.on("close", () => {
      ...
      if (commands) {
        console.log(
          chalk.yellow(`Comandos de git que contienen "${searchString}":`)
        );
        ...
        console.error(
          chalk.red(
            `No se encontraron comandos de git que contengan "${searchString}"`
          )
        );
      }
    });
    ...
    awk.on("error", (err) => {
      console.error(
        chalk.red(`Error al ejecutar el comando awk: ${err.message}`)
      );
    });
